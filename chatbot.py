...
def start_handler(message):
    if 'if_journey_info_needed' not in contexts:
        ...
        bot.send_message(message.chat.id, 'Hello, {}!'.format(new_friend),
                         reply_markup=chatbot_markup.yes_no_menu)
        time.sleep(SHORT_TIMEOUT)
        bot.send_photo(message.chat.id, 'https://iuriid.github.io/img/ft-1.jpg', caption='My name is <strong>{}</strong>. I\'m a traveler.\nMy dream is to see the world'.format(OURTRAVELLER), parse_mode='html')
        time.sleep(SHORT_TIMEOUT)
        bot.send_message(message.chat.id, 'Do you want to know more about my journey?', reply_markup=chatbot_markup.yes_no_menu)
...
def text_handler(message):
    ...
    # Block 5. User clicked button 'You got Teddy?' and was prompted to enter the secret code
        ...
        pass
        # here goes function that checks the code against the one from DB
    ...
    # Without context - feed callback_data from button to Dialogflow, return result and menu
        ...
        # Button 'You got Teddy?'
            ...
            bot.send_message(call.message.chat.id,
                             'Oh, that\'s a tiny adventure and some responsibility;)\nTo proceed please enter the <i>secret code</i> from the toy',
                             parse_mode='html')
            bot.send_photo(call.message.chat.id, 'https://iuriid.github.io/img/ft-3.jpg', reply_markup=chatbot_markup.intro_menu)
            contexts.clear()
            contexts.append('enters_code')
